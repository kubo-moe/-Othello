...
void decision(int a, int b, int myturn, int opponent, int space, int counter,
  ...
  do {
    if (counter == 0) {
      up = 0, spare = 1, side = -1, reserve = 0;  //下の判定
    } else if (counter == 1) {
      up = -2, spare = -1, side = -1, reserve = 0;  //上の判定
    } else if (counter == 2) {
      up = -1, side = 0, spare = 0, reserve = 1;  //右の判定
    } else if (counter == 3) {
      up = -1, side = -2, spare = 0, reserve = -1;  //左の判定
    }
